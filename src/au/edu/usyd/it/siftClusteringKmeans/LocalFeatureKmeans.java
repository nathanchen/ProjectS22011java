			...
			try
			{
				...
				for(String str : dir.list())
				{
					if(str.endsWith("_1.jpg_sift.txt"))
					{
						...
						System.out.println(str.substring(0, str.lastIndexOf("_1.jpg_sift.txt")));
						map.put(index, str.substring(0, str.lastIndexOf("_1.jpg_sift.txt")));
						index ++;
						/*
...
//						histo.append(res.substring(0, res.lastIndexOf(",")) + "\n");
				...
				for(int i = 0; i < totalNumOfFrames; i ++)
				{
					...
					if((i+1) % 3 == 0)
					{
						histo.append(normalise(result.substring(0,  result.lastIndexOf(","))) + "\n");
						result = "";
					}
				}
				histo.close();
...
//				k.setDistanceFunction(new CosineSimilarity());
				DataSource src = new DataSource(DIR + "siftHisto.arff");
				...
				for(int i = 0; i < NUMCLUSTERS; i++)
				{
					...
					for(int j = 0; j < arr.length; j++)
					{
						if(arr[j] == i)
						{
							String imgFile = map.get(j);
							System.err.println(imgFile);
							for(int n = 1; n <=3; n++)
							{
								File ori = new File(DIR + imgFile + "_" + n + ".jpg");
								File des = new File(DIR + i + "/" + imgFile + "_" + n + "。jpg");
	...
	private static String normalise(String str)
	{
		...
		double[] real = new double[numbers.length];
		double total = 0;
		String result = "";
		for(int i = 0; i < numbers.length; i++)
		{
			real[i] = Double.parseDouble(numbers[i]);
			total = total + real[i];
		}
		for(int i =0; i < numbers.length; i++)
		{
			result = result + real[i]/total + ",";
		}
